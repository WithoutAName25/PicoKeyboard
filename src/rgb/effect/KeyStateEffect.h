...
public:
    ...
    KeyStateEffect(T0 inactiveEffect, T1 activeEffect, const uint64_t fadeOutMs, const bool mirrored)
        : IRGBEffect(EffectType::KEY_STATE), inactiveEffect(std::make_shared<T0>(std::move(inactiveEffect))),
          activeEffect(std::make_shared<T1>(std::move(activeEffect))), fadeOutMs(fadeOutMs), mirrored(mirrored) {}
