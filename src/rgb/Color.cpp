...
Color::Color(const uint8_t red, const uint8_t green, const uint8_t blue)
    : r(red), g(green), b(blue), h(0), s(0), v(0), rgb_valid(true), hsv_valid(false), none(false) {}

Color::Color(const uint16_t hue, const float saturation, const float value)
    : r(0), g(0), b(0), h(hue % 360), s(std::clamp(saturation, 0.0f, 1.0f)), v(std::clamp(value, 0.0f, 1.0f)),
      rgb_valid(false), hsv_valid(true), none(false) {}

Color Color::FromRGB(uint8_t red, uint8_t green, uint8_t blue) { return {red, green, blue}; }

Color Color::FromHSV(uint16_t hue, float saturation, float value) { return {hue, saturation, value}; }
...
uint32_t Color::toPixelFormat() {
    if (!rgb_valid)
        updateRGB();
    return (static_cast<uint32_t>(g) << 24) | (static_cast<uint32_t>(r) << 16) | (static_cast<uint32_t>(b) << 8);
}
...
Color Color::withBrightness(const float brightness) const {
    ...
    if (hsv_valid)
        return {h, s, v * b};
    return {static_cast<uint8_t>(r * b), static_cast<uint8_t>(g * b), static_cast<uint8_t>(this->b * b)};
}

bool Color::isNone() const { return none; }
...
Color Color::interpolate(const Color &from, const Color &to, const float t) { return interpolateHSV(from, to, t); }
