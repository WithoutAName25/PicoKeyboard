...
class CommandController final : public IExecutable {
    ...
    template <DerivedFromCommand Cmd> void listenFor() {
        commandMap[Cmd::ID] = [](InterDeviceCommunicator &comm) { return std::make_unique<Cmd>(comm); };
    }
...
public:
    ...
    template <DerivedFromCommand Cmd, typename... Args> void send(Args &&...args) {
